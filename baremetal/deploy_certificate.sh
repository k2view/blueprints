...
function _microk8s() {
  ...
  if ! microk8s="$(command -v "microk8s")"; then
...
function restart_ingress() {
  ...
  while ! $microk8s kubectl -n $ingress get all | grep -q "^pod/nginx-ingress-microk8s-controller.*Running"; do
    sleep 10
  done
}
...
function deploy_cert_precheck() {
  ...
  if [[ -z "$privatekey" ]]; then
    err="arguments missing, 2 required: /path/to/fullchain.cer /path/to/private.key"
...
if [[ -n "$err" ]]; then
  echo "$err"
  echo "Usage:"
  echo "  $self_name /path/to/fullchain.cer /path/to/private.key"
  exit 1
fi
