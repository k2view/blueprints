...
function install_k8s() {
  ...
  linux)
    ...
    ;;
  MacOS)
    if [[ $(minikube status | grep -c Running) -eq 3 ]] &>/dev/null; then
      minikube status
    else
...
" | _tee ${rollingTextFile}
  ...
  linux)
    ...
    ;;
  MacOS)
    for addon in ingress metrics-server registry volumesnapshots csi-hostpath-driver; do
...
function presetup_k2agent() {
  ...
  get_input_with_default 'k2cloud Mailbox URL' 'https://cloud.k2view.com/api/mailbox'
  MANAGER_URL="${INPUT_WITH_DEFAULT}"
  get_input_with_default 'k2cloud Mailbox ID' 'no default - this should be provided by K2view'
